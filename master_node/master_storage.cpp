...
std::string MasterStorage::addNode(Message::node node, Report::hardware_result hardware, Message::node *monitored) {
    ...
    if(node.id == "") 
    {
        ...
        query << "SELECT id,ip,port FROM MNodes WHERE ip = \""+ node.ip+"\" AND port = \""+node.port+"\" LIMIT 1";
        ...
        if(nodes.size() > 0) {
            ...
            query << "SELECT id,ip,port FROM MNodes ORDER BY id DESC LIMIT 1";
            int err = sqlite3_exec(this->db, query.str().c_str(), VectorNodeCallback, &nodes, &zErrMsg);
            if( err!=SQLITE_OK )
            {
                fprintf(stderr, "SQL error (get node %s): %s\n", query.str().c_str(),zErrMsg);
...
void MasterStorage::complete() {
    ...
    std::sprintf(buf,
                ...
                " SELECT A.id AS idA, B.id AS idB, "
                    ...
                    "   (SELECT avg(M.meanB) as meanB from MLinks AS M WHERE (M.idA = A.id AND M.lasttimeB <> 0) UNION "
                    "   SELECT avg(M.meanB) as meanB from MLinks AS M WHERE (M.lasttimeB <> 0 AND M.idB = B.id)))"
                    "AS meanB, "
                    "(SELECT max(varianceB) from "
                    "   (SELECT avg(M.varianceB) as varianceB from MLinks AS M WHERE (M.idA = A.id AND M.lasttimeB <> 0) UNION "
                    "   SELECT avg(M.varianceB) as varianceB from MLinks AS M WHERE (M.lasttimeB <> 0 AND M.idB = B.id)))"
