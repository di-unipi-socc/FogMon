...
void MasterStorage::complete() {
    ...
    std::sprintf(buf,
                ...
                " SELECT A.id AS idA, B.id AS idB, "
                    ...
                    "   (SELECT min(M.meanB) as meanB from MLinks AS M WHERE (M.idA = A.id AND M.lasttimeB <> 0) UNION "
                    "   SELECT min(M.meanB) as meanB from MLinks AS M WHERE (M.lasttimeB <> 0 AND M.idB = B.id)))"
                    "AS meanB, "
