...
class Experimenter:
    ...
    def clear_session(self, session, _all=False):
        link = "remove"
        if _all:
            link = "removeall"
        r = self.connect("GET",f"http://{interface}/testbed/{session}/{link}",200)
        if r is None:
            raise Exception("connection error")
    ...
    def start_moment(self, name, apply=True):
        spec = self.spec.spec
    ...
    def wait_stability(self, num=10, check=False):
        stable = False
        ...
        while not stable:
            ...
            try:
                ...
                if int(re.search(r'\d+', moment["stable"]).group()) >= num:# or "False (3)" in moment["stable"]:
                    stable = True
                    self.get_roles()
                if check and ((moment["base"]["B3"]["mean"]>1 and (moment["time"] > 550 or "False (3)" in moment["stable"])) or moment["time"] > 1200):
                    return False
            except:
                pass
    ...
    def start_experiment_nodes(self, conf, name):
        ...
        leaders = 2 if l_nodes < 25 else 3 if l_nodes < 35 else 4
    ...
    def start_experiment_links_test(self):

        # self.start_experiment_nodes(configs["default"],"defaults")
        # self.start_experiment_num_leaders(configs["default"],"defaults")
        ...
        while not end:
            ...
            self.start_fogmon(configs[name])
            self.wait_stability()

            self.change_links(5,100,500)
            end = self.wait_stability(check=True)
            self.pull()
            self.stop_fogmon()
            self.restore_links(moment=False)
            if not end:
                self.clear_session(self.sessions[-1]["session"], _all=True)
                del exp.sessions[-1]
    ...
    def start_experiment_links_test2(self):
    ...
    def start_experiment_num_leaders(self, conf, name):
        ...
        conf["--leader-formula"] = 9 if l_nodes < 25 else 11 if l_nodes < 35 else 13
        self.start_session(f"leaders 1 {name}")

        self.start_fogmon(conf)
        self.wait_stability()
        self.stop_fogmon()
        self.start_moment("stop",apply=False)

        # conf = copy.deepcopy(base_conf)
        # conf["--leader-formula"] = 4 if l_nodes < 25 else 5 if l_nodes < 35 else 6
        # self.start_session(f"leaders 2 {name}")
        ...
        conf = copy.deepcopy(base_conf)
        conf["--leader-formula"] = 2 if l_nodes < 25 else 3 if l_nodes < 35 else 4
        self.start_session(f"leaders 3 {name}")
        ...
        if command == "test2":
            ...
            with Experimenter(topology) as exp:
                with open("../sessions.json","r") as rd:
                    exp.sessions = json.load(rd)
                exp.restore_links(moment=False)
                exp.start_experiment_links_test()
        with Experimenter(topology) as exp:
            if command == "setup":
                exp.setup()
                ...
                exp.start_experiment_num_leaders(configs[config],config)
            elif command == "nodes2":
                with open("../sessions.json","r") as rd:
                    exp.sessions = json.load(rd)
                exp.start_experiment_nodes(configs[config],config)
                ...
                with open("../sessions.json","r") as rd:
                    exp.sessions = json.load(rd)
                exp.clear_session(exp.sessions[-1]["session"], _all=True)
                del exp.sessions[-1]
                with open("../sessions.json","w") as wr:
                    json.dump(exp.sessions, wr)
            elif command == "moment":
                spec = exp.spec.spec
                r = exp.connect("POST",f"http://{interface}/testbed/{config}",201, json=spec)
                moment = r.json()["moment"]
                print("moment",moment)